...
namespace S2Please.Areas.ADMIN.Controllers
{
    public class MenuController : BaseController
    {
        ...
        public MenuController(ITableRepository tableRepository, IOrderRepository orderRepository, ISystemRepository systemRepository, IProductRepository productRepository , IMenuRepository menuRepository, IProductGroupRepository productGroupRepository)
        {
        ...
        public ActionResult Index()
        {
            ...
            var responseTableUser = _tableRepository.GetTableUserConfig(TableName.Menu, CurrentUser.UserAdmin.USER_ID);
            ...
            {
                var responseOrder = _tableRepository.GetTableByTableName(TableName.Menu);
            ...
            menu.Table.TABLE_NAME = TableName.Menu;
            menu.Table.TITLE_TABLE_NAME = FunctionHelpers.GetValueLanguage("Table.Title.Menu");
        ...
        public ActionResult ShowFormSaveMenu(long id = 0, bool isUpdate = true)
        {
            ...
            if (id!=0 && !isUpdate)
            {
                vm.Menus = vm.Menus.Where(s => s["ID"] != id).ToList();
            }

            var responseProductGroup = _productGroupRepository.GetAllProductGroup();
            vm.Groups = responseProductGroup.Results;

            var responseMultipleLanguage = _systemRepository.GetTableMultipleLanguageConfigurationByTableName(TableName.Menu);
            ...
            if (id != 0)
            {
                ...
                var responseMenuById= _menuRepository.GetMenuById(id);
            ...
            var html = RenderViewToString(this.ControllerContext, "~/Areas/ADMIN/Views/Menu/_FormSaveMenu.cshtml", vm);
        ...
        public ActionResult SaveMenu(MenuModel model, List<LocalizationModel> localiza)
        {
            ...
            var modelType = MapperHelper.Map<MenuModel, Repository.Model.MenuModel>(model);
            var localizaType = MapperHelper.MapList<LocalizationModel, Repository.Type.LocalizationType>(localiza);
            var result = new ResultModel();
            var response = _menuRepository.SaveMenu(modelType, localizaType);
            if (response != null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            result.SetDataMessage(true, FunctionHelpers.GetValueLanguage("Message.UpdateFail"), FunctionHelpers.GetValueLanguage("Message.Success"), string.Empty);
                        }
                    }
        ...
        public ActionResult Delete(long id = 0)
        {
            ...
            var response = _menuRepository.Delete(id);
            if (response.Success == false && CheckPermision(response.StatusCode) == false)
            {
                result.SetUrl("/Base/Page404");
            }
        ...
        public ActionResult ReloadTable(TableViewModel tableData, ParamType param)
        {
            ...
            if (tableName == TableName.Menu)
            {
                tableData.TITLE_TABLE_NAME = FunctionHelpers.GetValueLanguage("Table.Title.Menu");
            }
        ...
        private TableViewModel RenderTable(TableViewModel tableData, ParamType paramType)
        {
            ...
            {
                tableData.TABLE_URL = TableUrl.Menu;
                tableData.MENU_NAME = MenuName.Menu;
                tableData.TABLE_EXPORT_URL = TableExportUrl.Menu;
                tableData.TABLE_SESION_EXPORT_URL = TableSesionExportUrl.Menu;
            }
        ...
        public TableViewModel GetData(TableViewModel tableData, ParamType paramType)
        {
            ...
            if (tableData.TABLE_NAME == TableName.Menu)
            {
                var response = _menuRepository.GetMenuFromAdmin(type);
                if (response != null)
                {
                    if (response.Success == false && CheckPermision(response.StatusCode) == false)
                    {
